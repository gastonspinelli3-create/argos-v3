...
def get_opps(x_tenant_token: Optional[str] = Header(None, alias="X-Tenant-Token")):
    if not ok_token(x_tenant_token or ""):
        raise HTTPException(status_code=401, detail="Invalid token")
    return [{"id":"CASE_001_MIDLAND_FIREBIRD_54K","basin_id":"midland","county":"Upton","facts":[{"type":"water_production_real","description":"5000 bpd real"}],"verification_status":"PARTIALLY_VERIFIED","decision":"HOLD","temporal_claim":"excess 1500 bpd -> delta 2625 $/day"}]
